...
- Если вы хотите **узнать информацию из огромных файлов**, например, получить множество объектов из файла сохранения, стоит использовать `XMLParser:ReadFromBigfile()`.

- Если вы читаете **большие файлы, но хотите их редактировать**, настоятельно рекомендую рассмотреть вариант с "очередью": `XMLParser:openQueue()` и `XMLParser:closeQueue()`. Он будет наиболее "оптимизированным" вариантом, который лучше справляется с большими файлами - игра заметно меньше страдает. Этот вариант сложнее поддается на проверки.
